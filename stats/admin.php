...
/////////////////////////////////
if (!$option['clear_cache']) { // Controllo se non si � forzato il riconoscimento continuo - Check for continuous forced recognition
...
if ($cache_recn == 1 && (user_is_logged_in() || (!$option['use_pass'] && in_array($action, $unlockedPages)))) {
    ...
    // Visualizzo il login se non si � loggati o il logout se lo si �. - Show login if not logged in or logout if logged in
    if ($is_loged_in) {
    ...
    /*** Controllo validit� dei file di browscap */
